...
static int rk_v1_rng_read(struct udevice *dev, void *data, size_t len)
{
	...
	reg = CRYPTO_V1_OSC_ENABLE | CRYPTO_V1_TRNG_SAMPLE_PERIOD(100);
	writel(reg, pdata->base + CRYPTO_V1_TRNG_CTRL);

	reg = HIWORD_UPDATE(CRYPTO_V1_RNG_START, CRYPTO_V1_RNG_START, 0);

	writel(reg, pdata->base + CRYPTO_V1_CTRL);
...
exit:
	/* close TRNG */
	writel(HIWORD_UPDATE(0, CRYPTO_V1_RNG_START, 0),
	       pdata->base + CRYPTO_V1_CTRL);
...
static int rk_v2_rng_read(struct udevice *dev, void *data, size_t len)
{
	...
	writel(HIWORD_UPDATE(reg, 0xffff, 0), pdata->base + CRYPTO_V2_RNG_CTL);
...
exit:
	/* close TRNG */
	writel(HIWORD_UPDATE(0, 0xffff, 0), pdata->base + CRYPTO_V2_RNG_CTL);
